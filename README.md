...
## FIT-style photorealistic preprocessing

See [`REALISTIC_IMAGE_GENERATION.md`](REALISTIC_IMAGE_GENERATION.md) for the
complete GCDv2 render-to-photo workflow, texture-diversity experiment, measured
medium-quality token usage, cost calculation, and differences from FIT.

`src/tools/fit_vto_api.py` provides OpenAI-compatible replacements for the
proprietary Gemini and Nano Banana calls used by the FIT data-generation
pipeline. It reads the **second-to-last** non-empty credential from
`/scratch/m000133/george/foveated_diffusion_3d/key.txt`, where the inference
`sk-` key currently precedes the final IHub management credential. It never
copies either credential into this repository.

The configured routes are:

- `https://inference-api.nvidia.com/v1` with `openai/openai/gpt-image-2` for
  image generation and image editing
- `https://inference-api.nvidia.com/openai/v1` with
  `azure/openai/gpt-5.6-luna` for garment captions, measurements, and QA

These are intentionally separate API roots. When using the OpenAI Python SDK,
pass the API root as `base_url`; do not append `/chat/completions` because the
SDK appends the operation path itself. GPT Image 2 must be invoked with
`client.images.generate(...)` or `client.images.edit(...)`, not Chat
Completions.

Check which image deployments the credential can see before running an image
job:

```bash
python src/tools/fit_vto_api.py models --api image --contains image
```

Model-list visibility does not guarantee that a credential is entitled to use
that deployment. Supply a different position with
`--credential-index-from-end` if the key-file ordering changes. The tool
deliberately does not fall back to another image model.

Run commands from the repository root:

```bash
# Verify the Luna Responses request format.
python src/tools/fit_vto_api.py respond \
  --prompt "Hello! Can you help me?"

# Text-to-image using the exact JSON request fields: model, prompt, n, size.
python src/tools/fit_vto_api.py generate \
  --prompt "A beautiful sunset in the background, along with children playing." \
  --output outputs/fit_vto/sunset.png

# FIT paper's Nano Banana replacements (these use /v1/images/edits because
# preserving an input render or person requires image-to-image editing).
python src/tools/fit_vto_api.py fit-head-shoes \
  --image path/to/synthetic_render.png \
  --hair-style "short wavy" \
  --shoe-type "white sneakers" \
  --output outputs/fit_vto/head_shoes.png

python src/tools/fit_vto_api.py fit-try-off \
  --image path/to/photorealistic_tryon.png \
  --output outputs/fit_vto/layflat.png

python src/tools/fit_vto_api.py fit-paired-person \
  --image path/to/person.png \
  --garment-description "a loose blue short-sleeved cotton shirt" \
  --output outputs/fit_vto/paired_person.png

# Convert a synthetic GCDv2/Blender garment render into a photorealistic
# full-body image while locking its silhouette, construction, and colors.
python src/tools/fit_vto_api.py fit-photorealistic \
  --image path/to/synthetic_render.png \
  --garment-description "a pale-aqua strapless fitted bodice and matching calf-length A-line skirt" \
  --prompt-output outputs/fit_vto/realistic_prompt.txt \
  --output outputs/fit_vto/realistic.png

# Produce a new surface treatment without preserving the source material/color.
python src/tools/fit_vto_api.py fit-retexture \
  --image outputs/fit_vto/realistic.png \
  --texture-description "deep green silk velvet with gold botanical embroidery" \
  --quality medium \
  --usage-output outputs/fit_vto/retextured_usage.json \
  --prompt-output outputs/fit_vto/retexture_prompt.txt \
  --output outputs/fit_vto/retextured.png

# FIT paper's Gemini replacements. Luna receives the image as a base64 data URL
# inside the otherwise standard Responses API request.
python src/tools/fit_vto_api.py fit-caption \
  --image path/to/render.png \
  --format released-code \
  --output outputs/fit_vto/caption.txt

python src/tools/fit_vto_api.py fit-measurements \
  --image path/to/person.png \
  --output outputs/fit_vto/measurements.txt

python src/tools/fit_vto_api.py fit-qa \
  --image path/to/render.png \
  --output outputs/fit_vto/qa.json
```

Override `--key-file`, `--image-base-url`, `--text-base-url`, `--image-model`,
or `--text-model` when running against a different OpenAI-compatible
deployment. The tool does not replace FIT's Sapiens normal/segmentation
estimation or its FLUX retexturing LoRA; it only replaces the external
image-editing and VLM calls.
