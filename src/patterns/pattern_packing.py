"""Dependency-light packing for semantically arranged sewing-pattern panels.

This module owns the geometry and hierarchy logic used to lay pattern panels
out in a plane. Callers provide plain arrays and a small tree-like hierarchy;
the implementation has no dependency on a garment-generation framework,
texture representation, renderer, or mesh library.
"""
...
_DEFAULT_PANEL_TREE_PATH = (
    Path(__file__).resolve().parents[1] / "assets" / "panel_tree.json"
)
...
def load_panel_hierarchy(path=None):
    ...
    if path is None:
        ...
        if not hierarchy_path.is_file():
            # Preserve support for callers that provide the project asset from
            # their working directory instead of beside the source package.
...
class GarmentPackingPolicy:
    ...
    rules used by the semantic layout: front/back sections separate vertically,
    torso halves and pant legs separate horizontally, and sleeves move away
    from torsos horizontally.
    """
