...
class NNSewingPattern(VisPattern):
        ...
        for idx in range(len(pattern_representation)):
            ...
            try:
                self.panel_from_numeric(
                    ...
                    padded=padded, valid_mask=valid_mask[idx] if valid_mask is not None else None)
    ...
    def panel_from_numeric(self, panel_name, edge_sequence, rotation=None, translation=None, padded=False, valid_mask=None):
        ...
        if rotation is not None:
            if rotation.shape[-1] == 3: 
                ...
                rotation = scipy_rot.from_euler('xyz', rotation, degrees=False).as_quat()
