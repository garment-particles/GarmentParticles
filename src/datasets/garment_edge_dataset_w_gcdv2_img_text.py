...
class GarmentEdgeDatasetWithGCDV2Image(GarmentEdgeDataset):
    def __init__(
        self, 
    ...
    def __getitem__(self, idx):
        ...
        name = file.split("/")[-2]
        gcd_folder = os.path.dirname(self.gcd_specs[name])

        img_paths = [os.path.join(gcd_folder, f"{name}_render_front.png"), os.path.join(gcd_folder, f"{name}_render_back.png")]
