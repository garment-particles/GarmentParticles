...
class GarmentParticleDatasetV2WithImageText(GarmentParticleDatasetV2):
    def __init__(
        self, 
        data_dir, 
        normalization,
        ...
        self.gcd_list_file = gcd_list_file
        ...
        self.gcd_folders = {garment_name: os.path.join(gcd_dir, name) for name, garment_name in zip(gcd_list, garment_names)}
        self.front_only = front_only
        self.front_back = front_back
    def __getitem__(self, idx):
        file = self.files[idx]
        name = file.split("/")[-2]
        img_paths = [os.path.join(self.gcd_folders[name], f"{name}_render_front.png"), os.path.join(self.gcd_folders[name], f"{name}_render_back.png")]
