...
import random
import h5py
class GarmentParticleDatasetV2WithText(GarmentParticleDatasetV2):
    def __init__(
        self, 
        data_dir, 
        text_path,
    ...
    def __getitem__(self, idx):
        ...
        # randomly select a subset of captions
        ...
        if random.random() < self.text_drop_prob:
            caption = ""
        encoding = self.tokenizer(caption, max_length=self.max_length, padding="max_length", truncation=True, return_tensors="pt")
        tokens = encoding["input_ids"].squeeze(0)
        attn_mask = encoding["attention_mask"].squeeze(0).bool()
        ...
        if self.pad_everything:
            ...
            pcd = pcd[:self.n_points]
        mask = None
        if not self.fixed_n_points and self.pad_everything:
    ...
    def reconstruct(self, noise, mask=None, text_tokens=None, text_attn_mask=None, **kwargs):
        imgs, pcds, img_data = super().reconstruct(noise, mask, **kwargs)
        N = text_tokens.shape[0]
        captions = []
        for i in range(N):
            caption = self.tokenizer.decode(text_tokens[i], skip_special_tokens=True)
            captions.append(caption)
        return imgs, pcds, img_data, captions, []
...
if __name__ == "__main__":
    ...
    dataset = GarmentParticleDatasetV2WithText(
        data_dir="/orion/u/w4756677/garment/gcdv2/garment_particles_v2.1_11182025",
        text_path="/orion/u/w4756677/garment/gcdv2/short_captions_v2.json",
        tokenizer_name="openai/clip-vit-large-patch14",
        max_length=77,
        text_drop_prob=0.5,
    ...
    print(batch_dict["text_tokens"].shape)
    print(batch_dict["text_attn_mask"].shape)
    print(batch_dict["cu_input_lens"].shape)
    N = batch_dict["text_tokens"].shape[0]
    print(N)
