...
class Panel:
    ...
    def as_vector(self):
        ...
        for edge in self.edges:
            panel_vecs.append(edge.as_vector())
        return np.stack(panel_vecs)
    
    def normalize(self, scale, shift):
        if len(self.panel_vertices) > 0:
            self.panel_vertices = (np.array(self.panel_vertices) - shift) / scale
            self.panel_vertices = self.panel_vertices.tolist()
        self.corner_vertices = (self.corner_vertices - shift) / scale
        self.edges = []
        for edge in np.asarray(self.panel_dict['edges']):
            edge_obj = Edge(edge, self.corner_vertices, self.mesh_resolution)
            self.edges.append(edge_obj)
        
    def remesh_to_npoints(self, n_points):
        if len(self.panel_vertices) <= n_points:
            return
        area = np.abs(igl.doublearea(np.array(self.panel_vertices), np.array(self.panel_faces))).sum() / 2
        target_area = area / n_points
        target_edge_length = math.sqrt(target_area * 1.5) 
        for edge in self.edges:
            edge.init_curve(edge.edge_params, target_edge_length)
            
        self.panel_vertices = []
        self.panel_faces = []

        #Sort self.edges by stitch id
        n_stitch_edges, sorted_edges = self.sort_edges_by_stitchid()

        for i,(edge_id,edge) in enumerate(sorted_edges):
            #Get vertices for edge (without start, end)
            edge_in_vertices = edge._get_edge_in_verts(plot = False)

            #Store start, inside, and end vertices to Panel.panel_vertices and indices to edge.sitch_range
            self.store_edge_verts(edge, edge_in_vertices)

            if i == n_stitch_edges - 1:
                self.n_stitches = len(self.panel_vertices)# until now we only have stitch vertices in Panel.panel_vertices

        #Set panel norm
        self.set_panel_norm()
        #Generate panel mesh and store them in self.panel_vertices and self.panel_faces
        self.gen_panel_mesh(target_edge_length)
    ...
    def gen_panel_mesh(self, mesh_resolution, plot=False, check=False, normalize_before_tri=False): 
        """
...
class Edge:
    ...
    def as_vector(self):
        edge_vector = np.zeros(2+2+2+1) # diff, control1, control2, curvature_type
        start, end = self.endpoints
        diff = end - start
        edge = self.edge_params
        edge_vector[:2] = diff
        ...
        if 'curvature' in edge:
            if isinstance(edge['curvature'], list) or edge['curvature']['type'] == 'quadratic':  # NOTE: placeholder for old curves for backward compatibility
                ...
                edge_vector[2:4] = cubic_1
                edge_vector[4:6] = cubic_2
            elif edge['curvature']['type'] == 'cubic':
                edge_vector[2:4] = np.array(edge['curvature']['params'][0])
                edge_vector[4:6] = np.array(edge['curvature']['params'][1])
                ...
                print(params[0], params[1], params[2], start, end)
                _, _, local_coords = arc_rad_flags_to_three_point(start, end, params[0], params[1], params[2])
                edge_vector[2:4] = local_coords
                edge_vector[4:6] = local_coords
                edge_vector[6] = 1
    ...
    def _get_edge_in_verts(self, plot=False):
        """
        ...
        if isinstance(self.curve, svgpath.QuadraticBezier) or isinstance(self.curve, svgpath.CubicBezier):
             # to achieve equal spread along bezier curve
            curve_lengths = np.linspace(0,1,n) * self.curve.length()
            t_vals = [self.curve.ilength(c_len) for c_len in curve_lengths]

        ts = t_vals[1:(n - 1)]  # remove start and end from "inside vertices"
        if isinstance(self.curve, svgpath.Arc):
            for t in ts:
                p = pat_utils.c_to_np(self.curve.point(t))
                edge_in_vertices.append(p)
        else:
            points = self.curve.points(ts)  # faster than .point(t) but unavailable for Arc
        ...
        if plot:
            ...
            if isinstance(self.curve, svgpath.QuadraticBezier) or isinstance(self.curve, svgpath.CubicBezier):
                c_type = "bezier"
            elif isinstance(self.curve, svgpath.Line):
                c_type = "linear"

            show_verts = np.array([self.endpoints[0]] +list(edge_in_vertices) + [self.endpoints[1]])
...
class BoxMesh(wrappers.VisPattern):
    ...
    """
    def __init__(self, path, res=1.0, normalize=False):
    ...
    def regen_panel_meshes(self, npionts_panel):
        for panelname in self.panelNames:
            panel = self.panels[panelname]
            panel.remesh_to_npoints(npionts_panel)
    ...
    def finalise_mesh(self):
        ...
        * Creates stitch segmentation
        ...
        for panelname in self.panelNames:
            ...
            self.vertices_with_name.extend([panel.panel_name] * len(v_3D_non_stitch))
    ...
    def get_box_mesh_uv(self, in_uv_config={}, mat_name='panels_texture', out_texture_image_path=None, out_fabric_tex_image_path=None, out_mtl_file_path=None):
        if not self.loaded:
        ...
        uvs, materials, uv_dict, max_uv, min_uv = texture_mesh_islands(
            vertex_texture=self.vertex_texture,
            panel_tree=self.panel_tree,
            out_texture_image_path=out_texture_image_path,
            out_fabric_tex_image_path=out_fabric_tex_image_path,
            out_mtl_file_path=out_mtl_file_path,
            ...
            mat_name_prefix=mat_name
        )
        for vertex_texture in self.vertex_texture:
            vertex_texture['uv'] = uv_dict[vertex_texture['panel_name']]
        
        # Build panel_to_material mapping using is_front method
        panel_to_material = {}
        for panel_name in self.panelNames:
            group = 'front' if self.is_front(panel_name) else 'back'
            if group in materials:
                panel_to_material[panel_name] = materials[group]['name']
            else:
                # Fallback to first available material if group not found
                panel_to_material[panel_name] = list(materials.values())[0]['name']
        
        return uvs, panel_to_material, max_uv, min_uv
    ...
    def save_box_mesh_obj(self, with_normals=False, in_uv_config={}, mat_name='panels_texture'):
        ...
        """
        uvs, panel_to_material, max_uv, min_uv = self.get_box_mesh_uv(in_uv_config, mat_name)
        ...
        save_obj_with_materials(
            ...
            self.faces_with_texture, 
            uvs, 
            vertex_texture_info=self.vertex_texture,
            panel_names=self.faces_with_name,
            panel_to_material=panel_to_material,
