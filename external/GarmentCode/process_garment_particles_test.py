...
def process_garment_single(pattern_folder, out_folder):
    ...
    for side, islands in [('front', front_islands), ('back', back_islands)]:
        ...
        for i, island in enumerate(islands):
            ...
            for edge_id, edge in enumerate(panel.edges):
                ...
                for j in range(len(edge_verts)-1):
                    ...
                    lines.append(line)
                    inside_pts.append(line(eps * judge_orientation(panel_name, is_front)))
                    if target_edge is None:
                        outside_pts.append(line(-eps * judge_orientation(panel_name, is_front)))
                    else:
                        # dart 
                        if is_dart_and_skip:
                            continue
        ...
        if len(all_inside_pts) > 0:
            plt.scatter(all_inside_pts[:, 0], all_inside_pts[:, 1], color='red', label='inside', s=0.5, alpha=0.6)
        if len(all_interior_verts) > 0:
            plt.scatter(all_interior_verts[:, 0], all_interior_verts[:, 1], color='green', label='interior', s=0.5, alpha=0.6)
        if len(all_outside_pts) > 0:
            plt.scatter(all_outside_pts[:, 0], all_outside_pts[:, 1], color='blue', label='outside', s=0.5, alpha=0.6)
        plt.axis('equal')
        plt.legend()
        plt.tight_layout()
        ...
        plt.figure(figsize=(10, 10))
        plt.triplot(all_verts[:, 0], all_verts[:, 1], all_faces, alpha=0.7)
        plt.axis('equal')
        plt.tight_layout()
        plt.savefig(out_folder / f'garment_particles_init_tri_{side}.png', dpi=150, bbox_inches='tight')
        plt.close()

        inside_pts, outside_pts = optimize_particles(all_verts, all_inside_pts, all_outside_pts, thresh=2)
            
        plt.figure(figsize=(10, 10))
        plt.scatter(inside_pts[:, 0], inside_pts[:, 1], s=0.1, alpha=0.6)
        plt.scatter(outside_pts[:, 0], outside_pts[:, 1], s=0.1, alpha=0.6)
        plt.axis('equal')
        plt.tight_layout()
        ...
        regionA, regionB = bichromatic_voronoi_separator(optimized_inside_pts, outside_pts, bbox=None, pad=0.2)
        ...
        for geom, alpha, c in [(regionA, 0.1, "red"), (regionB, 0.1, "blue")]:
            ...
            if geom.geom_type == "Polygon":
                ...
                ax.fill(xs, ys, alpha=alpha, color=c)
                if c == "red":
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="garment_particles_rerun_rerun")
