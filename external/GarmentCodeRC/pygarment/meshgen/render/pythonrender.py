...
def load_meshes(paths: PathCofig, body_v, body_f):
    ...
    # Check if mesh has valid UVs for texturing
    has_uvs = (
        hasattr(garm_mesh.visual, 'uv') and 
        garm_mesh.visual.uv is not None and 
        len(garm_mesh.visual.uv) > 0
    )

    if has_uvs and isinstance(garm_mesh.visual, trimesh.visual.TextureVisuals):
        # Material adjustments
        material = garm_mesh.visual.material.to_pbr()
        material.baseColorFactor = [1., 1., 1., 1.]
        material.doubleSided = True  # color both face sides  
        ...
        # Fallback: no UVs, use a simple material
        fallback_material = pyrender.MetallicRoughnessMaterial(
            baseColorFactor=(0.8, 0.8, 0.8, 1.0),
            metallicFactor=0.0,
            roughnessFactor=0.5,
            doubleSided=True
        )
        pyrender_garm_mesh = pyrender.Mesh.from_trimesh(garm_mesh, material=fallback_material, smooth=True)
